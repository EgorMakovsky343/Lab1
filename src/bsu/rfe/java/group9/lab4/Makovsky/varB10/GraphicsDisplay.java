package bsu.rfe.java.group9.lab4.Makovsky.varB10;
...
// Шрифт для подписей осей координат
        ...
        labelsFont = new Font("Serif", Font.PLAIN, 12); // Initialize labelsFont in constructor
    }
...
// Шаг 9 - Восстановить старые настройки холста
    ...
    protected void paintGraphics(Graphics2D canvas) {
        float[] dashPattern = {20f, 5f, 3f, 5f, 10f, 5f, 3f, 5f, 20f, 5f};
        dashedStroke = new BasicStroke(1f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, dashPattern, 0.0f);
        ...
        for (int i = 0; i < graphicsData.length; i++) {
            ...
            // Display y-value near the point
            String label = String.format("%.2f", graphicsData[i][1]);
            FontRenderContext context = canvas.getFontRenderContext();
            Rectangle2D bounds = labelsFont.getStringBounds(label, context);
            canvas.drawString(label, (float)(point.getX() + 5), (float)(point.getY() - 5));
        }
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            // Координаты для крестика и квадрата (одинаковые для обоих)
            double x1 = center.x - 5.5;
            double y1 = center.y - 5.5;
            double x2 = center.x + 5.5;
            double y2 = center.y + 5.5;

            Line2D.Double line1 = new Line2D.Double(x1, y1, x2, y2);
            Line2D.Double line2 = new Line2D.Double(x1, y2, x2, y1);

            Rectangle2D.Double square = new Rectangle2D.Double(x1, y1, 11, 11);

            double yValue = point[1];
            double nearestInteger = Math.round(yValue); //округление

            // Установка цвета в зависимости от условия
            if (Math.abs(yValue - nearestInteger) < 0.1) {
                canvas.setColor(Color.GREEN);
            } else {
                canvas.setColor(Color.RED); // Ставим красный цвет для остальных маркеров
            }
...
// Создать объект контекста отображения текста - для получения характеристик устройства (экрана)
        labelsFont = new Font("Serif", Font.PLAIN, 12);
        ...
        for (double x = minX; x <= maxX; x += xStep) {
            Point2D.Double start = xyToPoint(x, 0);
            Point2D.Double end;
            if (Math.abs(x / xStep % 5) < 1) {
                end = xyToPoint(x, 0.02 * (maxY-minY));
                String label = String.format("%.2f", x);
                Rectangle2D bounds = labelsFont.getStringBounds(label, context);
                canvas.drawString(label, (float) (start.x - bounds.getWidth() / 2), (float) (start.y + bounds.getHeight() + 5));
            } else {
                end = xyToPoint(x, 0.01 * (maxY-minY));
            }
        ...
        for (double y = minY; y <= maxY; y += yStep) {
            Point2D.Double start = xyToPoint(0, y);
            Point2D.Double end;
            if (Math.abs(y / yStep % 5) < 1) {
                end = xyToPoint(0.02 * (maxX-minX), y);
                String label = String.format("%.2f", y);
                Rectangle2D bounds = labelsFont.getStringBounds(label, context);
                canvas.drawString(label, (float) (start.x - bounds.getWidth() - 5), (float) (start.y + bounds.getHeight() / 4));
            } else {
                end = xyToPoint(0.01 * (maxX-minX), y);
            }
            canvas.draw(new Line2D.Double(start, end));
        }
        }
